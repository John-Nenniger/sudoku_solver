...
def row_elimination(grid, x)
    grid[x].each_with_index do |number, y|
        if number == 0
            p generate_possibilities_for_a_point(x, y)[0]
        end
    end
end

row_elimination(initial, 0)

# p generate_possibilities_for_a_point(0, 5)


# I need to to add in the ability to check for a number within a section,
# see if that number can't be placed anywhere else in a section 
# then place it in the current location if it doesn't already exist and can't go anywhere else